        ...
        NavigationStack {
            ZStack {
                ...
                List {
                    ...
                    ForEach(store.entries) { entry in
                        Button {
                            draft = EntryDraft(date: entry.date, temperature: entry.temperature, notes: entry.notes)
                            editingID = entry.id
                            showingAdd = true
                        } label: {
            ...
            .sheet(isPresented: $showingAdd) {
                AddEntrySheet(draft: $draft, isEditing: editingID != nil) {
                    if let id = editingID {
                        store.updateEntry(id, date: draft.date, temperature: draft.temperature, notes: draft.notes)
                    } else {
                        store.addEntry(date: draft.date, temperature: draft.temperature, notes: draft.notes, isPro: purchases.isPro)
                    }
                    showingAdd = false
                }
            }
